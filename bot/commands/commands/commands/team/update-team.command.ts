...
import { isTeamCaptain } from '../../../../utils/permission.utils';
import { createErrorEmbed, createSuccessEmbed, StatusIcons } from '../../../../helpers/message.helpers';

export const handleUpdateTeam = async (interaction: ChatInputCommandInteraction): Promise<void> => {
  ...
  try {
    ...
    const isCaptain = await isTeamCaptain(interaction.user.id, team.teamId);
    if (!isCaptain) {
      await interaction.editReply({
        embeds: [
          createErrorEmbed(
    ...
    if (!success) {
      await interaction.editReply({
        embeds: [
          createErrorEmbed(
            'Update Failed',
            'Failed to update the team. Please try again later.',
          ),
        ],
      });
      return;
    ...
    logger.info(
      `Team "${teamName}" (${team.teamId}) updated by ${interaction.user.tag} (${interaction.user.id})`,
    );
  } catch (error) {
